...
function activeCompare() {
  if (productCompare = false) {
    return;
      ...
      let getCompareBtnFilter = document.querySelectorAll(".js-compare-btn");
      let getProductCompareLocalStorageIdFilter = JSON.parse(localStorage.getItem("data-product-handle"));

      if (getProductCompareLocalStorageIdFilter) {
        ...
        for (let i = 0; i < productshandleList.length; i++) {
          for (let k = 0; k < getCompareBtnFilter.length; k++) {
            if (getCompareBtnFilter[k].getAttribute("data-product-handle") === productshandleList[i]) {
              getCompareBtnFilter[k].classList.add("compareIn");
            }
          }
        }
      }
...
function renderProductSortView () {
  if ( typeof jQuery.cookie('productSortView') == 'undefined' ) {
    jQuery.cookie('productSortView', 'grid', {path: '/'});
  }
    ...
    jQuery('#product_listing__sorted').addClass('product_listing__list').removeClass('product_listing__grid product_listing__grid_large product_listing__grid_small');
  } else if ( jQuery.cookie('productSortView') == 'grid' ){
    jQuery('#view_list, #view_grid_large, #view_grid_small').removeClass('active');
    jQuery('#view_grid').addClass('active');
    jQuery('#product_listing__sorted').addClass('product_listing__grid').removeClass('product_listing__list product_listing__grid_large product_listing__grid_small');
    ...
    jQuery('#product_listing__sorted').addClass('product_listing__grid_large').removeClass('product_listing__list product_listing__grid product_listing__grid_small');
    ...
    jQuery('#product_listing__sorted').addClass('product_listing__grid_small').removeClass('product_listing__list product_listing__grid product_listing__grid product_listing__grid_large');
  }
}